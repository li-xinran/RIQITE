...
test_that( "CI methods work", {
    ...
    res <- calc_CI_precision_finite( dat$Y0, dat$tau, 0.5, R=5, summarise = FALSE, nperm = 200 )
    expect_true( is.matrix(res) )
    expect_equal( dim( res ), c( 150, 5 ) )
    ...
    res3 <- calc_CI_precision_finite( dat$Y0, dat$tau, 0.5, R=5, summarise = TRUE, nperm = 200,
...
test_that( "explore_stephenson_s_finite CI works", {
    ...
    res <- explore_stephenson_s_finite( s = c(2, 5), Y0 = dat$Y0, tau = dat$tau, R = 10,
                                        k.vec = 140:150,
