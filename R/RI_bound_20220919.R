...
#' @param alternative A character takes value "greater" and "less" indicating the direction of the alternative hypothesis.
#' @param method.list A list specifies the choice of the rank sum test
#'   statistic. For example, list(name = "DIM") means the difference-in-means statistic, 
#'   list(name="Wilcoxon") means the Wilcoxon rank sum statistic, 
#'   and list(name = "Stephenson", s = 10) means the Stephenson rank sum statistic with parameter s=10.
...
#'   approximating the randomization distribution of the test statistic
#' @param impute A character specifies the form of the test statistic. "control" means we use
#' the imputed control potential outcomes in the test statistic, and "both" means we use the 
#' imputed observed outcome in the test statistic. 
#' @return The p-value for testing the specified null hypothesis of interest.
#' @export
pval_bound <- function(Z, Y, c=0, method.list = list(name = "DIM"), alternative = "greater", Z.perm = NULL, nperm = 10^5, impute = "control"){
...
#' the imputed control potential outcomes in the test statistic, and "both" means we use the 
#' imputed observed outcome in the test statistic. 
#' @param alpha A numeric object specifies the level of the confidence interval. In particular, the confidence level is \eqn{1-\alpha}. 
