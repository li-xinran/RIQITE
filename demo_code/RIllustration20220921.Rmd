...
s_selector = mutate( s_selector,
                     group = cut( k, 
                                  breaks = c(0, 233-70, 203-30, 223-10, 233),
...
s_selector %>% filter( group != "v low" ) %>%
    ...
    geom_smooth( aes( col=tx_dist ), method = "loess", se = FALSE, 
                 span = 1, lwd= 0.5 ) +
    labs( x = "s", y = "Median lower CI bound" ) +
...
avg %>% ungroup() %>%
    ...
    dplyr::select(-q_ci) %>%
    pivot_wider( names_from="rho", values_from="s", 
                 names_prefix = "rho=" ) %>%
    knitr::kable()
```
...
Across these results, $s = 6$ looks like a reasonable choice, as the "low" group does not appear to generally be informative.
