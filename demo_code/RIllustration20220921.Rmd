---
title: "**Real data analysis and power analysis using the developed package RIQITE**"
...
Below we will also use the package to analyze the real data sets in the paper. 
```{r, message=FALSE}
devtools::install_github("li-xinran/RIQITE")
```


# Testing monotonicity of an strumental variable
...
ggplot(data, aes(EntryAge, color = BirthMonth, linetype=BirthMonth)) +
  ...
  annotate("text", x=c(7.2, 7.2), y=c(.145, .81), label=c("January", "December")) +
  scale_x_continuous(breaks=seq(6, 8, .2)) + theme_bw() + 
  theme( plot.background = element_blank(), panel.grid.major = element_blank(), 
         panel.grid.minor = element_blank() ) + theme(plot.title=element_text(hjust=.5)) + 
...
We get the data from @HSMHSD10. We remove units with missing outcomes, as well as units in Site 1 where there are no control outcomes. 
...
library(RIQITE)
## DATA
dat = read_csv( "cleanTeacher.csv",na = "." )
...
n = nrow(dat)
table(dat$TxAny)
print(paste("estimated average effect is", 
            round(lm(gain~TxAny, dat)$coefficients[2], digits = 2)))
```
...
hist(dat$gain[dat$TxAny==1], xlim = range(dat$gain), freq = FALSE, ylim = c(0, 0.065), 
     col = "grey", border=FALSE, breaks = 20, main = NULL, xlab = "gain score")
hist(dat$gain[dat$TxAny==0], freq = FALSE, add = TRUE, breaks = 20, col = NULL)
```

We first test the bounded null hypothesis that all individual effects are non-positive. 
```{r}
nperm = 10^6
pval.steph10 <- pval_quantile(Z=dat$TxAny, Y=dat$gain, k=n, c=0, alternative = "greater", 
                              method.list=list(name="Stephenson", s=10), nperm=nperm )
print(paste("The p-value is", pval.steph10))
```
...
ci10 = ci_quantile( Z=dat$TxAny, Y=dat$gain, alternative="greater", 
                  method.list=list( name="Stephenson", s=10 ), nperm=nperm, alpha=0.10 )
ci2 = ci_quantile( Z=dat$TxAny, Y=dat$gain, alternative="greater", 
                  method.list=list( name="Stephenson", s=2 ), nperm=nperm, alpha=0.10 )
par(mfrow = c(1, 2))
for(s in c(10, 2)){
  if(s==10){ci.limit = ci10$lower}
  if(s==2){ci.limit = ci2$lower}
  plot( NA, ylab="k", xlab=expression("lower"~"confidence"~"limit"~"for" ~tau[(k)]), 
        ylim=c(102, 233)+c(-1, 1), xlim=range( ci.limit[ci.limit>-Inf] )+c(-0.5, 1.5),
        main=paste0("s=", s) )
  for(k in 1:length(ci.limit)){
    lines( c( max( ci.limit[k], min(ci.limit[ci.limit>-Inf]) - 100 ), max(ci.limit)+10),
           rep(k,2), col = "grey" )
  }
  points( ci.limit, c(1:length(ci.limit)), pch = ".", cex = 2 )
  abline(v = 0, lty = 2)
}
```
...
```{r}
tau.max.lower = ci_quantile( Z=dat$TxAny, Y=dat$gain, k.vec=n, alternative="greater", 
              method.list=list( name="Stephenson", s=10 ), nperm=nperm, alpha=0.05 )$lower
tau.min.upper = ci_quantile( Z=dat$TxAny, Y=dat$gain, k.vec=1, alternative="less", 
...
library(foreign)
library(RIQITE)
# source("power_simu_20220919.R")
# source("RI_Quantile_20220919.R")
# source("RI_Max_20220919.R")
dat = read.spss("Data file, 24.09.2021.sav", to.data.frame=TRUE)
```
...
par(mfrow = c(1, 2))
for(s in c(6, 2)){
  if(s==6){ci.limit = ci6$lower}
  if(s==2){ci.limit = ci2$lower}
  plot(NA, ylab="k", xlab=expression( "lower"~"confidence"~"limit"~"for" ~tau[(k)] ), 
       ylim=c(n-37, n)+c(-1, 1)*1, xlim=range(ci.limit[ci.limit>-Inf])+c(-0.5, 1.5), 
       main = paste0("s=", s))
  for(k in 1:length(ci.limit)){
    lines( c( max( ci.limit[k], min(ci.limit[ci.limit>-Inf]) - 100 ), max(ci.limit)+10), 
           rep(k,2), col = "grey" )
